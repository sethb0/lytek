...
export function getTitle (type, group) {
  ...
  case 'Infernal':
    ...
    if (group === 'Martial Arts') {
      return 'Infernal Martial Arts Charms';
    }
    return `Charms of ${group === 'Ebon Dragon' ? 'the ' : ''}${group}`;
  case 'Knacks':
    return 'Lunar Knacks';
  case 'Jadeborn':
    return `Jadeborn ${group} Pattern`;
  case 'Terrestrial Martial Arts':
  case 'Celestial Martial Arts':
  case 'Sidereal Martial Arts':
    return group.includes(' Style') ? group : `${group} Charms`;
  default:
    return `${type} ${group}${group.includes(' Charms') ? '' : ' Charms'}`;
  }
}
